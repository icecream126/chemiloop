...
wandb.init(project=f"tartarus_v5", name=f"{task}",config=vars(args))#  , mode='disabled')
...
def retrieval_node(state: GraphState) -> GraphState:
    ...
    if task == "emitters":
        for data in dataset:
            smiles = data["smiles"]
            st = data.get("singlet-triplet value", 0)
            osc = data.get("oscillator strength", 0)
            combined = data.get("multi-objective value", 0)
            topk_smiles.append((smiles, st, osc, combined))
    else:
        for data in dataset:
            smiles = data["smiles"]
...
def scientist_node(state: GraphState) -> GraphState:
    ...
    task = state["task"][0] if type(state["task"])==list else state["task"] 
    if task=="emitters":
        topk_smiles = utils.utils.format_topk_smiles_emitters(state["topk_smiles"])
    else:
...
def reviewer_node(state: GraphState) -> GraphState:
    ...
    if mol is None:
        ...
        if task=="emitters":
            st, osc, combined = 0, 0, 0
        else:
            score = 0.0
        state["scientist_think"]["smiles"] += "(This SMILES is invalid, please retry.)"
    else:
        if task=="opv_pce_pcbm":
            dipm, gap, lumo, combined, pce_pcbm_sas, pce_pcdtbt_sas = TASK_TO_SCORING_FUNCTION[task](state['generated_smiles'])
            score = pce_pcbm_sas
        elif task=="opv_pce_pcdtbt":
            dipm, gap, lumo, combined, pce_pcbm_sas, pce_pcdtbt_sas = TASK_TO_SCORING_FUNCTION[task](state['generated_smiles'])
            score = pce_pcdtbt_sas
        elif task=="emitters":
            st, osc, combined = TASK_TO_SCORING_FUNCTION[task](state['generated_smiles'])
        else:
    ...
    if task=="emitters":
        state["smiles_scores"].append((state["generated_smiles"], st, osc, combined))
        SMILES_log(SMILES+" , "+str(st)+" , "+str(osc)+" , "+str(combined))
        whole_SMILES_log(SMILES+" , "+str(st)+" , "+str(osc)+" , "+str(combined))
        log("== Score ==")
        log(f"st: {st}, osc: {osc}, combined: {combined}")
        wandb.log({
            "st": st,
            "osc": osc,
            "combined": combined,
...
"""
        ...
        score = f"""
Oscillator Strength (HIGHER IS BETTER): {osc}
Singlet-triplet energy gap (SMALLER IS BETTER): {st}
Combined (HIGHER IS BETTER): {combined}
"""
