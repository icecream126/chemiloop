...
wandb.init(project="json_1000_pmo_v2_isomers_c7h8n2o2", name="history_pmo")# , mode='disabled')
...
def scientist_node(state: GraphState) -> GraphState:
    global SMILES_HISTORY, MESSAGE_HISTORY
    print("\n==== Scientist Node ==")
    
    TEXT_SMILES_HISTORY = utils.utils.format_set_as_text(SMILES_HISTORY)
    if state["json_output"]:
        if state["reviewer_think"] == "":
            user_prompt = get_v2_json_prompts.get_scientist_prompt_isomers_c7h8no2(TEXT_SMILES_HISTORY)
        else:
            user_prompt = get_v2_json_prompts.get_scientist_prompt_with_review_isomers_c7h8n2o2(state["prompt"], state['scientist_think'], state['reviewer_think'], state["generated_smiles"], state["score"], state["functional_groups"], TEXT_SMILES_HISTORY)
        ...
        # Scientist's prompt by considering the reviewer's feedback
        if "albuterol_similarity" in state["target_prop"]:
            if state["reviewer_think"] == "":
                prompt = get_v2_prompts.get_scientist_prompt(state["prompt"])
            else:
                prompt = get_v2_prompts.get_scientist_prompt_with_review(state["prompt"], state['scientist_think'], state['reviewer_think'], state["generated_smiles"], state["score"], state["functional_groups"])
        elif "isomers_c7h8n2o2" in state["target_prop"]:
            if state["reviewer_think"] == "":
                prompt = get_v2_prompts.get_scientist_prompt_isomers_c7h8n2o2(state["prompt"])
            else:
                prompt = get_v2_prompts.get_scientist_prompt_with_review_isomers_c7h8n2o2(state["prompt"], state['scientist_think'], state['reviewer_think'], state["generated_smiles"], state["score"], state["functional_groups"])


        # Get scientist's response
...
def reviewer_node(state: GraphState) -> GraphState:
    ...
    SMILES_log(state["generated_smiles"]+" , "+str(score))
    ...
    if mol is not None:
        if "albuterol_similarity" in state["target_prop"]:
            ...
            functional_groups = utils.utils.count_atoms(mol)
    else:
    ...
    if state["json_output"]:
        ...
        user_prompt = get_v2_json_prompts.get_reviewer_prompt_isomers_c7h8n2o2(state["scientist_think"], score, functional_groups)
        prompt = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": user_prompt}
        ]
        ...
        if "albuterol_similarity" in state["target_prop"]:
            ...
            prompt = get_v2_prompts.get_reviewer_prompt_isomers_c7h8n2o2(state["prompt"], state['scientist_think'], score, functional_groups)
